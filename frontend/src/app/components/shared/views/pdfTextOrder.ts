 ...
 * The backend extractor reorders items this way before handing the text to the
 * model, so the viewer has to reorder them identically — otherwise a quote the
 * model produces cannot be found in the rendered text layer and the citation
 ...
 * column.
 *
 * MIRRORED FILE — keep in sync with `backend/src/lib/pdfTextOrder.ts`.
 *
...
function groupIntoRows(
    ...
    byBaseline.sort(
        (a, b) => items[b].y - items[a].y || items[a].x - items[b].x,
    );
